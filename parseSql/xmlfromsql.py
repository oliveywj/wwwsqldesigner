...
import sys
sys.path.insert(0,'/home/ubuntu/anaconda2/lib/python2.7/site-packages/sqlparse-0.2.2-py2.7.egg')
import sqlparse
...
sql_post= form.getvalue('sqlstring')
if sql_post:
    sql=sql_post.replace('#semicolon#',';')
...
for sqls in sql.splitlines():
    if (sqls.find('--')==-1) or (sqls.find('--')>5):
        sqlLines.append(sqls)

sql=""
for sqls in sqlLines:
    sql=sql+sqls+'\n'
